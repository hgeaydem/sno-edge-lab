...
build_bridges() {
  ...
  sudo nmcli con down "System $MAIN_CONN"
}
...
#  sleep 10
  ...
  get_oc_bin
  case $VERSION in
    sno) export RELEASE_IMAGE=quay.io/openshift-release-dev/ocp-release:4.8.0-fc.0-x86_64 ;;
  ...
  case $VERSION in
    sno)  oc adm release extract --command=openshift-install --to "${EXTRACT_DIR}" ${RELEASE_IMAGE} ;;
